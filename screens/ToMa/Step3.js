...
import { ToMaContext, UserContext } from "../../tools/Context";
...
export default function Step3({ navigation }) {
  ...
  >      
      ...
      <MenuButton
        ...
        title={"Angaben des Empfängers in Marokko"}
        textSize={13}
...
const InfoInputs = () => {
  const { formik, errors } = useContext(ToMaContext);
...
const SendingType = () => {
  const { formik, ChooseType } = useContext(ToMaContext);
  return (
    <>
...
const PaimentType = () => {
  const { formik, ChoosePayment } = useContext(ToMaContext);
...
const SexeHandler = () => {
  const { formik, PickGender, errors } = useContext(ToMaContext);

  return (
    <View>
      <GenderInputList Context={ToMaContext} Vname="mgender" />
      <HelperText
        type="error"
